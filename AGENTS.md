...
2. **Deploy the Convex backend BEFORE the builds** — `npx convex deploy -y` (production; the user has standing authorization, and the permission classifier needs the sandbox off). A binary that calls a function the deployment does not have crashes for every signed-in user the moment it renders, wherever that query lives. 1.0.34 shipped exactly that bug: `convex/followerActivities.ts` was committed but never deployed, `FollowerActivitySync` in the root layout queried it on every iOS screen, and App Review would have seen "Could not find public function for 'followerActivities:mine'". Deploying fixed the already-submitted build with no new binary, because the client only needed the function to exist. Run `npx convex dev --once` too, so the simulator's dev deployment matches.
