...
4. **Run the builds yourself** — the user has standing authorization for these commands and does not want to be asked to run them manually (use the global `eas` binary, not npx).
   **iOS builds LOCALLY; Android stays on EAS cloud** (user decision, 2026-09-21). The app links the iOS 27 SDK with the UIKit scene lifecycle (`expo-build-properties` `ios.enableSceneSupport`), which only this Mac's Xcode 27 provides — EAS Build has no Xcode 27 image yet, and a cloud iOS build (iOS 26 SDK) with scene support on has never shipped. Android has no such difference.
   `eas build -p ios --profile production --local --non-interactive --output <scratchpad>/FlyRight-<version>-<build>.ipa` then `eas submit -p ios --path <that ipa> --non-interactive`
   `eas build -p android --profile production --non-interactive --no-wait --auto-submit`
   Free ~15 GB first (`df -h /System/Volumes/Data`; the npm cache and `android/app/build` are safe to clear). Switch iOS back to the cloud command once EAS lists an Xcode 27 image (`docs.expo.dev/build-reference/infrastructure/`). Details: [release guide](docs/release-workflow.md) → Builds.
