...
- **The listing is shot signed in as the store profile** (since 1.1.0): Clerk **dev** user Maja Lindqvist (`maja+clerk_test@example.com`, OTP 424242, photo and name set in Clerk) with three synthetic friends, Noah (in the air), Clara (at security) and Tomas (landed), each with a postcard. Everything is saved in `scripts/store-profile.json`. Right before capturing, run `node scripts/seed-store-profile.mjs`: it resets the `store_demo_*` friends and re-anchors their trips, live stages and postcards to now on the dev deployment (the feed only shows the last 48 hours). Capture from a **Release build pointed at dev** (no `.env.production.local` present), sign in once per device (Settings → Sign in, `.maestro/clerk-dev-sign-in.yaml` with `EMAIL`), switch off Travel day live updates so the Dynamic Island stays empty, and open Friends once so its unseen badge clears. The friends never exist in production.
- Seed the viewer's own journal on each device with `node scripts/seed-demo-data.mjs --ios --sim <udid>` and `node scripts/seed-demo-data.mjs --android` (the seeded rows show for the signed-in viewer too). Add `--travel-day` for the upcoming-flight panel. The script gives all platforms the same nine-trip history anchored to now. Signed-in devices can pull real cloud trips over the seed, producing inconsistent panels.
