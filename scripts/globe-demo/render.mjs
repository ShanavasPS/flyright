...
// 1080 × 1350 feed video for the globe post: one iPhone on the right, the
// story on the left. Scenes come from edit.json — each names its source take
// (light or dark), the cut ([start, end] or several segments, sped to fit
// `duration`) and its two-line caption. Same recipe as scripts/detour-demo.
const out = resolve('demo/out/globe');
await mkdir(`${out}/graphics`, { recursive: true });
await mkdir(`${out}/segments`, { recursive: true });
const scenes = JSON.parse(await readFile(new URL(process.env.EDIT ?? './edit.json', import.meta.url), 'utf8'));
const phone = { x: 522, y: 172, width: 500, height: 1088, radius: 62 };
...
await sharp(Buffer.from(mask)).removeAlpha().png().toFile(`${out}/graphics/mask.png`);
...
for (const [index, scene] of scenes.entries()) {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1350">
    ...
    <circle cx="1010" cy="1180" r="620" fill="none" stroke="#71B7FB" stroke-opacity=".07" stroke-width="2" stroke-dasharray="4 13"/>
    <circle cx="1010" cy="1180" r="760" fill="none" stroke="#71B7FB" stroke-opacity=".05" stroke-width="2" stroke-dasharray="4 13"/>
    ...
    <g font-family="Helvetica Neue,Helvetica,Arial,sans-serif" fill="#F1F6FF">
      ...
      <text x="56" y="269" font-size="60" font-weight="700" letter-spacing="-2">The map</text>
      <text x="56" y="338" font-size="60" font-weight="700" letter-spacing="-2">is a globe now.</text>
      <text x="56" y="405" font-size="25" fill="#A9BBD2">One shader. No map SDK.</text>
      <rect x="56" y="515" width="62" height="5" rx="2.5" fill="#70B8FF"/>
      <text x="56" y="573" font-size="18" fill="#70B8FF" letter-spacing="3">${String(index + 1).padStart(2, '0')} / ${String(scenes.length).padStart(2, '0')} · WORLD TAB</text>
      ...
      <text x="56" y="892" font-size="24" fill="#70B8FF" font-weight="600">react-native-skia by Shopify</text>
      <text x="56" y="925" font-size="23" fill="#BAC9DD">Reanimated + Gesture Handler</text>
      <text x="56" y="955" font-size="23" fill="#BAC9DD">by Software Mansion</text>
      <text x="56" y="1000" font-size="26" font-weight="600">Scan to get FlyRight</text>
      <image href="data:image/png;base64,${qr.toString('base64')}" x="56" y="1020" width="${qrMeta.width}" height="${qrMeta.height}"/>
      <image href="data:image/png;base64,${appStore.toString('base64')}" x="298" y="1050" width="174" height="58"/>
      <image href="data:image/png;base64,${googlePlay.toString('base64')}" x="298" y="1142" width="${googlePlayMeta.width}" height="58"/>
      <text x="56" y="1268" font-size="24" font-weight="600">getflyright.com</text>
      <text x="56" y="1314" font-size="16" fill="#849BB7">iOS 26 simulator · FlyRight 1.0.37 · Expo SDK 57</text>
      ${scenes.map((_, j) => `<rect x="${1022 - (scenes.length - j) * 39}" y="1305" width="${j === index ? 29 : 11}" height="5" rx="2.5" fill="${j === index ? '#70B8FF' : '#3C5574'}"/>`).join('')}
    </g>
  ...
  const factor = scene.duration / sourceDuration;
  ...
  const filter = `${trims};${cat},setpts=${factor.toFixed(6)}*PTS,fps=30,tpad=stop_mode=clone:stop_duration=${scene.duration},scale=${phone.width}:${phone.height}:flags=lanczos,setsar=1,format=rgba[v];[2:v]format=gray[m];[v][m]alphamerge[screen];[0:v][screen]overlay=${phone.x}:${phone.y}:shortest=1,format=yuv420p[result]`;
  ff(['-loop', '1', '-framerate', '30', '-i', background, '-i', `${out}/${scene.source}`, '-loop', '1', '-framerate', '30', '-i', `${out}/graphics/mask.png`,
    '-filter_complex', filter, '-map', '[result]', '-t', String(scene.duration), '-an', '-c:v', 'libx264', '-preset', 'fast', '-crf', '18', '-movflags', '+faststart', `${out}/segments/${scene.id}.mp4`]);
...
ff(['-f', 'concat', '-safe', '0', '-i', `${out}/segments/list.txt`, '-c', 'copy', '-movflags', '+faststart', `${out}/flyright-globe.mp4`]);
await writeFile(`${out}/flyright-globe.srt`, captions.join('\n'));
// Cover: the fit globe with its comets, a second into the second scene.
ff(['-ss', String(scenes[0].duration + 1), '-i', `${out}/flyright-globe.mp4`, '-frames:v', '1', `${out}/flyright-globe-cover.png`]);
console.log(`Exported ${offset}s to ${out}/flyright-globe.mp4`);
