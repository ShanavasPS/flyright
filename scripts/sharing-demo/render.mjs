...
// Six pixels per module, including the four-module quiet zone on each side.
const qr = await QRCode.toBuffer(downloadUrl, { errorCorrectionLevel: 'M', margin: 4, scale: 6 });
await writeFile(`${out}/flyright-download-qr.png`, qr);
const appStore = await sharp(new URL('./assets/app-store.svg', import.meta.url).pathname, { density: 288 })
  .resize({ height: 58 }).png().toBuffer();
const googlePlay = await sharp(new URL('./assets/google-play.png', import.meta.url).pathname)
...
for (const [index, scene] of scenes.entries()) {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1350">
    ...
    <g font-family="Helvetica Neue,Helvetica,Arial,sans-serif" fill="#F1F6FF">
      ...
      <text x="56" y="269" font-size="60" font-weight="700" letter-spacing="-2">Your ticket.</text>
      <text x="56" y="338" font-size="60" font-weight="700" letter-spacing="-2">Your next trip.</text>
      <text x="56" y="405" font-size="25" fill="#A9BBD2">PDFs. Screenshots. Photos.</text>
      <rect x="56" y="515" width="62" height="5" rx="2.5" fill="#70B8FF"/>
      <text x="56" y="573" font-size="18" fill="#70B8FF" letter-spacing="3">${String(index+1).padStart(2,'0')} / ${String(scenes.length).padStart(2,'0')}</text>
      ...
      <text x="56" y="900" font-size="24" fill="#70B8FF" font-weight="600">expo-sharing</text>
      <text x="56" y="936" font-size="23" fill="#BAC9DD">Receive files on iOS + Android</text>
      ...
      <text x="56" y="1314" font-size="16" fill="#849BB7">Sample ticket · iOS app demonstration</text>
    ...
    <rect x="${phone.x-14}" y="354" width="5" height="65" rx="2" fill="#485A70"/>
    <rect x="${phone.x-14}" y="438" width="5" height="65" rx="2" fill="#485A70"/>
    <rect x="${phone.x+phone.width+10}" y="391" width="5" height="96" rx="2" fill="#485A70"/>
  </svg>`;
  ...
  // Normalize simctl's variable frame rate before trimming, so cuts within
  // static holds retain the screen that was visible at the requested time.
  ...
  execFileSync('ffmpeg',['-hide_banner','-loglevel','error','-y','-loop','1','-framerate','30','-i',background,'-i',`${out}/${scene.source}`,'-loop','1','-framerate','30','-i',`${out}/graphics/mask.png`,'-filter_complex',filter,'-map','[result]','-t',String(scene.duration),'-an','-c:v','libx264','-preset','fast','-crf','18','-movflags','+faststart',`${out}/segments/${scene.id}.mp4`],{stdio:'inherit'});
...
execFileSync('ffmpeg',['-hide_banner','-loglevel','error','-y','-f','concat','-safe','0','-i',`${out}/segments/list.txt`,'-c','copy','-movflags','+faststart',`${out}/flyright-share-to-trip.mp4`],{stdio:'inherit'});
await writeFile(`${out}/flyright-share-to-trip.srt`,captions.join('\n'));
execFileSync('ffmpeg',['-hide_banner','-loglevel','error','-y','-i',`${out}/flyright-share-to-trip.mp4`,'-frames:v','1',`${out}/flyright-share-to-trip-cover.png`],{stdio:'inherit'});
console.log(`Exported ${offset}s to ${out}/flyright-share-to-trip.mp4`);
