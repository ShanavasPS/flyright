...
/**
 * Camera viewfinder that reads the IATA BCBP barcode off a ticket or boarding
 * pass — PDF417 on printed passes and e-ticket receipts, Aztec/QR on phone
 * and watch screens. Parsing is pure (services/bcbp.ts); this component only
 * owns camera plumbing: permission, continuous-scan debounce, and the
 * not-a-boarding-pass hint. Dressed as the same night-sky pass card as the
 * rest of the add-flight flow.
 *
 * `onUpload`, when given, offers the same job done on a file instead: the
 * PDF the airline emailed or a screenshot of the pass, read by the document
 * importer. It is the way out for a pass the camera can't be pointed at.
 */
export function BoardingPassScanner({
  ...
  if (!permission.granted) {
    return (
      <PassCard>
        <MicroLabel>Scan ticket or pass</MicroLabel>
        <ThemedText type="smallBold" style={styles.title}>
          FlyRight needs the camera to scan
        </ThemedText>
        <ThemedText type="small" style={styles.hintText}>
          Point it at any ticket or boarding pass — paper or on a screen — and the
          flight fills itself in.
        </ThemedText>
        ...
        {onUpload && (
          <Pressable hitSlop={Spacing.two} onPress={onUpload}>
            <ThemedText type="smallBold" style={styles.link}>
