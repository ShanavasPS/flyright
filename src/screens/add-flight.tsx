...
import { addJourney, attachBoardingPass, saveImportedJourney, updateJourney, useJourney, useJourneys } from '@/services/journeys';
...
export function AddFlight({ step }: { step: Step }) {
  ...
  const applyScan = async (pass: BoardingPass, code: StoredPass | null) => {
    ...
    setScanning(false);
    if (!authLoaded || journal === undefined) {
      Alert.alert('Your trips are still loading', 'Please scan again in a moment so we can check for an existing flight.',
        [{ text: 'Scan again', onPress: () => setScanning(true) }, { text: 'Cancel', style: 'cancel' }]);
      return;
    }
    if (code) {
      ...
      try {
        const known = await confirmImportedJourney(segment, journal);
        if (known === 'cancel') return;
        if (known) {
          await saveImportedJourney(segment, null, userId, known.id);
