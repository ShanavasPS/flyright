...
describe('identifyCaller', () => {
  ...
  it('refuses a signed-out app request', async () => {
    const result = await identifyCaller(request());
  ...
  it('lets our web checker through when it identifies itself, keyed by address', async () => {
    ...
    const result = await identifyCaller(
      request({ 'x-flyright-web': '1', 'cf-connecting-ip': '203.0.113.7' }),
    );
  ...
  it('refuses another site that has no marker', async () => {
    ...
    const result = await identifyCaller(request({ origin: 'https://scraper.example' }));
    expect(result).toEqual({ ok: false, status: 401, error: 'sign_in_required' });
  });
  ...
  it('accepts a browser referrer from one of our sites without a marker', async () => {
    const result = await identifyCaller(request({ referer: 'https://getflyright.com/check' }));
    expect(result.ok).toBe(true);
  });

  it('refuses a referrer from anywhere else', async () => {
    const result = await identifyCaller(request({ referer: 'https://scraper.example/x' }));
    expect(result).toEqual({ ok: false, status: 401, error: 'sign_in_required' });
  });
  ...
  it('refuses a malformed or foreign token', async () => {
    expect(await identifyCaller(request({ authorization: 'Bearer not-a-jwt' }))).toEqual({
      ok: false,
      status: 401,
      error: 'invalid_token',
    });
    ...
    expect(await identifyCaller(request({ authorization: `Bearer ${header}.${claims}.c2ln` }))).toEqual({
      ok: false,
      status: 401,
      error: 'invalid_token',
    });
  ...
  it('fails closed in production when no issuer is configured', async () => {
    ...
    const result = await identifyCaller(request({ origin: 'https://getflyright.com' }));
