...
/** The store's introductory offer on a plan, worded for the plan card — "14 days
 * free" for a trial, "€1.99 for the first 3 months" for a paid intro — or null
...
export function planIntro(p: PurchasesPackage): string | null {
  ...
  const span = `${count} ${count === 1 ? name : `${name}s`}`;
  if (intro.price === 0) return `${span} free`;
  return `${intro.priceString} for the first ${span}`;
}
