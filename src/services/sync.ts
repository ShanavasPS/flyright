...
export async function applyRemoteJourney(remote: RemoteJourney, userId: string) {
  const columns = {
    ...
    terminal: remote.terminal ?? null,
    checkInDesk: remote.checkInDesk ?? null,
    gate: remote.gate ?? null,
    boardingTime: remote.boardingTime ?? null,
    baggageBelt: remote.baggageBelt ?? null,
    actualDeparture: remote.actualDeparture ?? null,
    actualArrival: remote.actualArrival ?? null,
    factsByUser: remote.factsByUser ?? null,
